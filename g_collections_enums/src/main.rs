...
impl Shot {
    ...
    fn points(self) -> i32 {
        ...
        /*
         * This is a match expression that takes a Shot enum and returns the points based on the variant.
         * The match expression is similar to a switch statement in other languages.
         * The match expression is followed by a block of arms.
         * Each arm has a pattern and an expression.
         * The pattern is compared to the value of the match expression.
         * If the pattern matches the value, the expression is evaluated.
         * The expression of the first arm that matches is returned.
         * If no arm matches, the match expression returns an error.
...
fn main() {
    ...
    for coord in arrow_coords {
        ...
        let shot = match coord.distance_from_center() {
            ...
            distance if distance >= 1.0 && distance <= 5.0 => Shot::Hit(distance),
            _ => Shot::Miss,
        };
        shots.push(shot);
